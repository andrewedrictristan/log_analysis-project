...
def get_favorite_articles(c):
    ...
    c.execute("select author_slugs.title,vs_article.total_visits "
              "from author_slugs,vs_article "
...
def get_favorite_authors(c):
    ...
    c.execute("select author_slugs.author_name,sum(vs_article.total_visits) "
              "as most_fav_author "
              "from author_slugs,vs_article where "
              "author_slugs.slug=vs_article.slug_path "
              "group by author_slugs.author_name order by "
...
def get_error_requests(c):
    ...
    c.execute("select rate,date "
              "from("
              "select cast((error_requests.error_req*100)"
              "/request_per_day.number_of_request "
              "as decimal(10,2))"
